...
var Calendar = fc.Calendar = fc.CalendarBase = Class.extend({
	...
	initOptions: function(overrides) {
		...
		// njcalendar change
		this.eventResources = options.resources;
		orderResources();
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		clipBoard = t.clipBoard new ClipBoard(t, options);
		menu = t.menu  new Menu(t, options, menuContainer);
		header = t.header = new Header(t, options);
		headerElement = header.render();
		if (headerElement) {
			element.prepend($("<div class='fc-header-space' />"));
	...
	function renderView(viewType, resourcesChanged) {
		ignoreWindowResize++;

		// if viewType is changing, destroy the old view
		if ((currentView && viewType && currentView.type !== viewType) || resourcesChanged) {
...
function EventManager(options) { // assumed to be a calendar
    ...
    */
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		//var isAmbigTimezone = t.getIsAmbigTimezone(); // njcalendar edit
		var undoFunctions = [];
...
var DragListener = fc.DragListener = Class.extend({
	...
	dragStart: function(ev) {
		...
		this.trigger('dragStart', ev); // njcalendar needed?
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	dayMousedown: function(ev) {
		...
		var dragListener = new CellDragListener(this.coordMap, {
			...
			cellOver: function(cell, isOrig, origCell, date) {
	...
	renderRangeHelper: function(range, sourceSeg, col) {
		var resources = view.calendar.getResources();
		var fakeEvent = this.fabricateHelperEvent(range, sourceSeg);
		fakeEvent.resource = resources[col] || resources[0];
...
Grid.mixin({
	...
	segDragMousedown: function(seg, ev) {
		...
		var dragListener = new CellDragListener(view.coordMap, {
			distance: 5,
			scroll: view.opt('dragScroll'),
			clipboardEl: calendar.getClipBoard().getClipBoardElement(),
			subjectEl: el,
			subjectCenter: true,
			listenStart: function(ev) {
			...
			dragStop: function(ev) {
				...
				mouseFollower.stop((!hasChanged && !clipboardDrop && !dropLocation), function() {
...
var TimeGrid = Grid.extend({
	...
	renderDrag: function(dropLocation, seg, col) {

		if (seg) { // if there is event information for this drag, render a helper event
			this.renderRangeHelper(dropLocation, seg, col);
	...
	// Renders a visual indication of an event being resized
	renderEventResize: function(range, seg, col) {
		this.renderRangeHelper(range, seg, col);
	},
	...
	renderSelection: function(range, col) {
		if (this.view.opt('selectHelper')) { // this setting signals that a mock helper event should be rendered
			this.renderRangeHelper(range, null, col);
		}
		else {
			this.renderHighlight(range, col);
		}
	},
...
var View = fc.View = Class.extend({
	...
	constructor: function(calendar, type, options, intervalDuration) {
		...
		this.isEventDraggable = isEventDraggable;
		this.isEventResizable = isEventResizable;
		this.eventDrop = eventDrop;
		this.eventResize = eventResize;
		this.workingHours = calendar.options.workingHours;
	...
	reportSelection: function(range ev, cell) {
		...
		this.trigger('select', null, range.start, range.end, ev, resourceObj);
	},
...
var AgendaView = fcViews.agenda = View.extend({
  ...
  // Sets the display range and computes all necessary dates
	...
	renderBusinessHours: function() {
		...
		View.prototype.destroy.call(this); // njcalendar is this needed? // call the super-method
	},
	...
	renderEvents: function(events) {
		...
		View.prototype.renderEvents.call(this, events); //njcalendar is this needed? // call the super-method
	},
	...
	bottomRuleHeight: null,
		...
		renderSelection: function(start, end, col) {
			if (start.hasTime() || end.hasTime()) {
				this.timeGrid.renderSelection(start, end, col);
			}
			else if (this.dayGrid) {
